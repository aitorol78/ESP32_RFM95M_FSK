...
#define TX_INTERVAL 2005
#define TX_EVERY_N_RECEIVED_PACKETS 50
...
static void rx_func (osjob_t* job) {
  ...
  if(LMIC.dataLen){
    iRxPacketCounter++;
    if(iRxPacketCounter == TX_EVERY_N_RECEIVED_PACKETS){
      os_setTimedCallback(&txjob, os_getTime(), tx_func); // TX _now_
      iRxPacketCounter = 0;
    }
...
static void txdone_func (osjob_t* job) {
  //Serial.print("TX time (os_ticks): "); 
  //Serial.println( os_getTime() - txStartTime);
  rx(rx_func);
}
...
static void tx_func (osjob_t* job) {
  // say hello
  char txLine[256];
  sprintf(txLine,"sending datagram number: %6u", iCounter++);
  tx(txLine, txdone_func);
  // reschedule job every TX_INTERVAL (plus a bit of random to prevent
  // systematic collisions), unless packets are received, then rx_func
  // will reschedule at half this time.
  //os_setTimedCallback(job, os_getTime() + ms2osticks(TX_INTERVAL + random(0*500)), tx_func);
}
...
# error Unsupported LMIC regional configuration.
  ...
  if(LMIC.rps == FSK){
    Serial.print("Mode: FSK (rps: "); Serial.print(LMIC.rps); Serial.println(" )");
  } else {
    Serial.print("Mode: Lora (rps: "); Serial.print(LMIC.rps); Serial.println(" )");
  }
